...
```text
工具调用 → 本地分流 → 独立模型审核（主模型 + 备用链）→ allow → omp 原生审批 → 执行
                                          └→ deny / ask / 整条链失败 → 单次人工确认
                                                                     └→ 拒绝、取消、无界面 → 阻止
```
...
`YOUR_PROVIDER/YOUR_MODEL_ID` 必须替换为 `omp models` 中实际存在的完整标识。建议选择与主 agent 不同的模型；插件只使用你明确配置的审核模型和备用链，不会切换主 agent 模型。相同模型的独立审核请求也可使用，但不属于跨模型复核。

模型和认证复用 omp 注册表，支持已有 provider、用户配置的 provider，以及其 API key/OAuth 认证解析。审核请求不携带工具定义。调用失败时先尝试配置的备用模型，全部失败后转人工确认。
...
| `fallbackModels` | `[]` | 按顺序尝试的备用审核模型，最多 8 个完整 `provider/model`；重复项会去重，不能包含主审核模型 |
| `reviewTimeoutMs` | `20000` | 整条审核模型链的总超时，包含认证 |
...
审核自动使用实际执行审核的模型声明支持的最低思考档位，按 `minimal < low < medium < high < xhigh < max` 选择，不依赖配置列表顺序；不支持推理或未声明可调档位的模型不传思考档位参数。已有档位映射由 OMP 处理。

主审核模型不存在、认证/接口异常、超时或返回无效响应时，按 `fallbackModels` 顺序切换。有效的 `allow`、`deny` 或 `ask` 都立即结束审核，不会因为拒绝而尝试其他模型。备用模型接收同一份脱敏后的审核上下文，各自使用自己的认证和最低思考档位。用户取消、切换会话或重载配置会取消整条链。

整条链共享 `reviewTimeoutMs`，每次尝试按剩余模型数均分剩余时间：例如总预算 20 秒且有一个备用模型，主模型最多先用约 10 秒；主模型快速失败时，备用模型可使用剩余时间。全部失败仍转人工确认，无交互界面则阻止。审计的 `model` 记录实际给出结论的模型（失败时记录最后尝试的模型，见下），备用成功时附带 `fallbackUsed: true`，确认框及最近审核状态也会标记备用模型。

例如在用户级 `auto-review.json` 中合并以下配置（替换为你已配置的模型）：

```json
{
  "model": "YOUR_PROVIDER/PRIMARY_MODEL",
  "fallbackModels": ["YOUR_PROVIDER/BACKUP_MODEL"]
}
```

修改后执行 `/auto-review reload`；`/auto-review status` 可查看备用链。

每次模型尝试的模型名、结果类别和耗时记录在审计的 `attempts` 数组中。链失败时，确认框显示最后尝试的模型，并列出各模型的失败原因（超时、模型未注册、认证失败、限流、响应格式无效、输出达到上限或接口异常）。原始接口错误和响应正文不写入诊断，避免泄露凭据。失败记录的 `model` 表示最后尝试模型；`fallbackUsed` 仍只表示备用模型成功给出了有效结论。没有开始模型尝试时不生成 `attempts`。

- `/auto-review status`：显示模型、备用链、配置路径、超时和覆盖边界。
- `/auto-review model <provider/model>`：验证模型存在并保存到用户配置文件；同名模型会同时从备用链移除，凭据由 omp 管理。
...
人工窗口展示审核模型、完整参数、工作目录、调用 ID 和原因，默认选择“拒绝执行”。人工选择时只有“仅批准本次调用”放行；取消、过期和不支持交互都会阻止。审批不缓存、不授权命令前缀，也不跨会话复用。新输入、会话切换或结束、配置重载会使待处理审批失效。

无交互模式（print/JSON/RPC/子 agent）只接受模型的 `allow`。第一版的人工窗口仅支持原生 TUI，不将确认请求转发给 ACP/RPC 客户端。

## 与 omp 原生审批并存

底部状态栏在审核结束后显示最近完成的一次审核结果及操作摘要，例如 `最近：审核通过 · bash: npm test`。人工放行、超时按建议自动放行或拒绝、审核拒绝和审核异常分别标记；备用模型给出结论时额外标注模型名；命令摘要会脱敏、转为单行并截断，文件内容不会显示。切换会话或重新加载审核配置时清空最近结果。这里的“审核通过”仅代表插件放行，不代表后续原生审批通过或命令执行成功。
...
审计文件位于 `getAgentDir()/auto-review/audit/YYYY-MM-DD.jsonl`，新目录权限为 `0700`，新文件为 `0600`。记录会话、调用 ID、参数字段名与字节数、SHA-256、审核模型（成功时为给出结论的模型，失败时为最后尝试的模型；备用模型另记 `fallbackUsed: true`）、各次尝试的 `attempts`、决定、脱敏原因、耗时及人工覆盖结果；ask 的推荐以脱敏后的 `recommendation` 字段保存，按推荐自动决策另记 `automaticRecommendation`；不写入原始命令、完整审核上下文或认证密钥。
...
单元测试覆盖风险分流、严格 JSON、上下文来源与脱敏、符号链接、配置、审计、异常、超时、取消、单次授权、推荐校验与超时自动决策、备用模型链与时间均分、失败尝试诊断、状态摘要和并发确认。

`test:omp` 会在临时目录复制一个**不含 node_modules** 的插件包，用真实 omp 和本地模拟 provider 验证批准、拒绝、无效响应、未配置模型、原生审批、普通子 agent、`write` 经 `xd://` 转发执行的审核、备用模型在主模型缺失/超时/响应无效时接管、有效裁决不触发切换、整条链失败转人工确认并记录各次尝试诊断，以及真实 TUI 的批准/默认拒绝/取消和按模型推荐自动放行/拒绝。模型由测试扩展在内存中模拟，不消耗付费模型额度；不修改用户的 omp 配置。受限子 agent 的不继承边界另有基于已锁定 SDK 源码的回归检查。

可设置 `OMP_BIN=/path/to/omp` 指定二进制，`OMP_SMOKE_CASES=child` 单独运行场景（`fallback-*` 为备用模型场景），`KEEP_SMOKE_ARTIFACTS=1` 保留测试产物；失败时自动保留产物路径。
